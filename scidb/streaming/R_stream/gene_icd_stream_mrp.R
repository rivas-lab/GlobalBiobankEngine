   ...
   "project(
      equi_join(
        ...
        project(
          equi_join(
            equi_join(
              project( filter( variant_ap,%s), rsid, ref, alt, exac_nfe, ukbb_freq),
              project(
                equi_join(
                  variant_gene,
                  equi_join(
    ...
    )", icd_filter, icd, variant_filter, gene_filter, bim_filter)
  result
}
...
gene_var_stream = function(f, icd = "HC125", variant_filter = "true", bim_filter="true", icd_filter="true", genes)
{
