                         ...
                         value_filter)
{
  ...
  variant_selection = sprintf("
    project(
     ...
     list_id, gene_name, chrom, pos, ref, alt, consequence
    )",
    variant_filter,
    gene_selection,
    bim_filter
  ...
  icd_selection = sprintf(
   "project(
      equi_join(
        ...
        project(
         %s,
         list_id, gene_name, chrom, pos, consequence
        ),
        'left_names=chrom,pos',
        'right_names=chrom,pos',
        'algorithm=hash_replicate_right'
      ),
      list_id, icd_idx, icd, gene_name, chrom, pos, lor, se, consequence
...
sample_function = function(gene_lookup_result)
{
 ...
 result$chr=gene_lookup_result$chrom
  result$pos=gene_lookup_result$pos
 result$ref = gene_lookup_result$ref
 result$alt = gene_lookup_result$alt
  ...
  gname = gene_lookup_result$gene_name
  ...
  csqs = as.factor(gene_lookup_result$consequence)
 # levels(csqs) <- 1:length(levels(csqs))
 levels(posids) <- 1:length(levels(posids))
 result$b <- b
#  posids <- as.numeric(posids)
  data <- list(b = b, ses = result$se, posids = posids, gene_name = gene_lookup_result$gene_name, gname = gname, csq = csqs)
#  mod1 <- brm(b | se(ses, sigma = TRUE) ~ 1 + csq + gname +  (csq|gname) +  (1|posids),  data = data, 
 # mod1 <- brm(b | se(ses, sigma = TRUE) ~ 1 + (csq|gname),  data = data, 
 #        cores = 4, control= list(adapt_delta=.99), save_all_pars = TRUE, family = "gaussian" )
 # pdf('/home/scidb/R_stream3/test1.pdf')
 # save.image('/home/scidb/R_stream3/mysession.Rda')
 # pp_check(mod1)
 # plot(mod1)
 # stanplot(mod1)
# plot(marginal_effects(mod1,probs = c(0.05, 0.95)))
# coefplot(mod1)
 # dev.off()
#  result$fixef = fixef(mod1)
#  result$ranef = ranef(mod1)
#  result$coef = coef(mod1)
                                             ...
                                             c('KRAS', 'PTEN', 'IDO1')))
{
   ...
   faster_redimension(
    apply( %s, 
     ...
     <list_id_dbl:double, icd_idx_dbl:double, icd:string, gene_name:string, chrom_dbl:double, 
      pos_dbl:double, lor:double, se:double, consequence:string>
  ...
  query = sprintf("
   stream(
    %s,
    'sudo -u scidb Rscript /home/scidb/R_stream3/gene_icd_stream_child.R',
    'format=df',
