...
library(data.table)
library(methods)
bb <- connect(username = "rivasadmin", password = "P@radigm4R1vasLab")
#bb <- connect(username = "scidbadmin", password = "Paradigm4")
ns_name <- "RIVAS_HG38"
as_name <- "UK_Biobank_Exome_European"
skip_list <- c()
...
directory <- "/opt/biobankengine/GlobalBioBankEngineRepo/gbe_data/ex_20210127_plink/"
pattern <- paste0(
  "([a-zA-Z0-9_]+)",
  ".metal.plink.tsv.gz")
