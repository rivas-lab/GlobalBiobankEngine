...
generated quantities {
	...
	//  vector[N] B_rep[M];
      ...
      for (n in 1:N){
      	  vector[K] p_raw;
	  for (k in 1:K){
	  // B_rep[n] = B_rep[n] + multi_normal_cholesky_rng(B[n],zeros, Sigmas[k] + diag_pre_multiply(SE_vec[n], L_Theta))*pi[k];
	  p_raw[k] <- pi[k]*exp(multi_normal_cholesky_log(B[n], zeros, Sigmas[k] + diag_pre_multiply(SE_vec[n], L_Theta)));
	  }
	  for (k in 1:K){
