...
app.config.update(dict(
    ...
    ICD_INFO_FILE=os.path.join(os.path.dirname(__file__), GBE_FILES_DIRECTORY, 'icdstats', 'icdinfoai.txt'),
    ICD_STATS_FILES=glob.glob(os.path.join(os.path.dirname(__file__), GBE_FILES_DIRECTORY, 'icdassoc','hybrid','c*.hybrid.gz')),
    QT_STATS_FILES=glob.glob(os.path.join(os.path.dirname(__file__), GBE_FILES_DIRECTORY, 'icdassoc','hybrid','c*.linear.gz')),
...
def load_gene_models():
    ...
    gwasdict = {}
    if os.path.exists('../gbe_data/gwas/'):
        gwasf = open('../gbe_data/gwas/diseasegwasinfo.txt','r').readlines()
        for gwasl in gwasf[1:]:
            gwasl = gwasl.rstrip()
            gwasl = gwasl.split('\t')
            gwasl = [i.strip('"') for i in gwasl]
            if len(gwasl) < 20:
                continue
            gene = gwasl[19].split(',')
            for geneid in gene:
                ...
                oddsratio = gwasl[17]
                gwaspvalue = gwasl[18]
                if (geneid,'disease') in gwasdict:
                    if disease == "IBD":
                        gwasdict[geneid,'disease'] = disease
                else:
                    gwasdict[geneid,'disease'] = disease
                if (geneid,'gwasindex') in gwasdict:
                    gwasdict[geneid,'gwasindex'].append(gwasindex)
                    gwasdict[geneid,'gwasvars'].append(gwasvars)
                    gwasdict[geneid,'gwasgenes'].append(gwasgenes)
                    gwasdict[geneid,'af'].append(af)
                    gwasdict[geneid,'oddsratio'].append(oddsratio)
                    gwasdict[geneid,'gwaspvalue'].append(gwaspvalue)
                    gwasdict[geneid,'gwasstart'].append(gwasstart)
                    gwasdict[geneid,'gwasend'].append(gwasend)
                else:
                    gwasdict[geneid,'gwasindex'] = [gwasindex]
                    gwasdict[geneid,'gwasvars'] = [gwasvars]
                    gwasdict[geneid,'gwasgenes'] = [gwasgenes]
                    gwasdict[geneid,'af'] = [af]
                    gwasdict[geneid,'oddsratio'] = [oddsratio]
                    gwasdict[geneid,'gwaspvalue'] = [gwaspvalue]
                    gwasdict[geneid,'gwasstart'] = [gwasstart]
                    gwasdict[geneid,'gwasend'] = [gwasend]
                if geneid in gwasl[15]:
                    gwasdict[geneid,'directlyimplicated'] = geneid

    lofdict = {}
    if os.path.exists('../gbe_data/lof/'):
        ibdf = open('../gbe_data/lof/bayesianmodel.ibd.noukbb.countsGENES.txt','r').readlines()
        for ibdl in ibdf:
            ibdl = ibdl.rstrip()
            ibdl = ibdl.split()
            gene = ibdl[0]
            ...
            src = ibdl[1]
            pvalue = float(ibdl[2])
            oddsratio = float(ibdl[3])
            vars = ibdl[4]
            nvars = ibdl[5]
            cnta = ibdl[6]
            cntu = ibdl[7]
            lofdict[gene,'AJnvar','ibd'] = -9
            lofdict[gene,'NFEnvar','ibd'] = -9
            lofdict[gene,'FINnvar','ibd'] = -9
            lofdict[gene,'AJcnta','ibd'] = -9
            lofdict[gene,'NFEcnta','ibd'] = -9
            lofdict[gene,'FINcnta','ibd'] = -9
            lofdict[gene,'AJcntu','ibd'] = -9
            lofdict[gene,'NFEcntu','ibd'] = -9
            lofdict[gene,'FINcntu','ibd'] = -9
            lofdict[gene,'totpvalue','ibd'] = pvalue
            lofdict[gene,'oddsratio','ibd'] = oddsratio
            src = src.split(';')
            for idx in range(0,len(src)):
                stnvars = nvars.split(';')
                stcnta = cnta.split(';')
                stcntu = cntu.split(';')
                lofdict[gene,str(src[idx]) + 'nvar','ibd'] = int(stnvars[idx])
                lofdict[gene,str(src[idx]) + 'cnta','ibd'] = int(stcnta[idx])
                lofdict[gene,str(src[idx]) + 'cntu','ibd'] = int(stcntu[idx])

        cdf = open('../gbe_data/lof/bayesianmodel.cd.noukbb.countsGENES.txt','r').readlines()
        for cdl in cdf:
            cdl = cdl.rstrip()
            cdl = cdl.split()
            gene = cdl[0]
            ...
            src = cdl[1]
            pvalue = float(cdl[2])
            oddsratio = float(cdl[3])
            vars = cdl[4]
            nvars = cdl[5]
            cnta = cdl[6]
            cntu = cdl[7]
            lofdict[gene,'AJnvar','cd'] = -9
            lofdict[gene,'NFEnvar','cd'] = -9
            lofdict[gene,'FINnvar','cd'] = -9
            lofdict[gene,'AJcnta','cd'] = -9
            lofdict[gene,'NFEcnta','cd'] = -9
            lofdict[gene,'FINcnta','cd'] = -9
            lofdict[gene,'AJcntu','cd'] = -9
            lofdict[gene,'NFEcntu','cd'] = -9
            lofdict[gene,'FINcntu','cd'] = -9
            lofdict[gene,'totpvalue','cd'] = pvalue
            lofdict[gene,'oddsratio','cd'] = oddsratio
            src = src.split(';')
            for idx in range(0,len(src)):
                stnvars = nvars.split(';')
                stcnta = cnta.split(';')
                stcntu = cntu.split(';')
                lofdict[gene,str(src[idx]) + 'nvar','cd'] = int(stnvars[idx])
                lofdict[gene,str(src[idx]) + 'cnta','cd'] = int(stcnta[idx])
                lofdict[gene,str(src[idx]) + 'cntu','cd'] = int(stcntu[idx])

        ucf = open('../gbe_data/lof/bayesianmodel.uc.noukbb.countsGENES.txt','r').readlines()
        for ucl in ucf:
            ucl = ucl.rstrip()
            ucl = ucl.split()
            gene = ucl[0]
            ...
            src = ucl[1]
            pvalue = float(ucl[2])
            oddsratio = float(ucl[3])
            vars = ucl[4]
            nvars = ucl[5]
            cnta = ucl[6]
            cntu = ucl[7]
            lofdict[gene,'AJnvar','uc'] = -9
            lofdict[gene,'NFEnvar','uc'] = -9
            lofdict[gene,'FINnvar','uc'] = -9
            lofdict[gene,'AJcnta','uc'] = -9
            lofdict[gene,'NFEcnta','uc'] = -9
            lofdict[gene,'FINcnta','uc'] = -9
            lofdict[gene,'AJcntu','uc'] = -9
            lofdict[gene,'NFEcntu','uc'] = -9
            lofdict[gene,'FINcntu','uc'] = -9
            lofdict[gene,'totpvalue','uc'] = pvalue
            lofdict[gene,'oddsratio','uc'] = oddsratio
            src = src.split(';')
            for idx in range(0,len(src)):
                stnvars = nvars.split(';')
                stcnta = cnta.split(';')
    ...
    with gzip.open(app.config['GENCODE_GTF']) as gtf_file:
        for gene in get_genes_from_gencode_gtf(gtf_file):
            ...
            if gene_id in canonical_transcripts:
                ...
                #print(gene['canonical_transcript'])
            ...
            if gene_id in dbnsfp_info:
                ...
                if (genesym,'disease') in gwasdict:
                    if genesym == "CARD9":
                        print(genesym,gwasdict[genesym,'gwasindex'])
                    gene['disease'] = gwasdict[genesym,'disease']
                    gene['gwasindex'] = gwasdict[genesym,'gwasindex']
                    ...
                    if (genesym,'directlyimplicated') in gwasdict:
                        gene['di'] = gwasdict[genesym,'directlyimplicated']
                if (genesym,'AJnvar','ibd') in lofdict:
                    gene['AJnvaribdlof'] = lofdict[genesym,'AJnvar','ibd']
                    gene['AJcntaibdlof'] = lofdict[genesym,'AJcnta','ibd']
                    gene['AJcntuibdlof'] = lofdict[genesym,'AJcntu','ibd']
                    gene['FINnvaribdlof'] = lofdict[genesym,'FINnvar','ibd']
                    gene['FINcntaibdlof'] = lofdict[genesym,'FINcnta','ibd']
                    gene['FINcntuibdlof'] = lofdict[genesym,'FINcntu','ibd']
                    gene['NFEnvaribdlof'] = lofdict[genesym,'NFEnvar','ibd']
                    gene['NFEcntaibdlof'] = lofdict[genesym,'NFEcnta','ibd']
                    gene['NFEcntuibdlof'] = lofdict[genesym,'NFEcntu','ibd']
                    gene['totpvalueibdlof'] = lofdict[genesym,'totpvalue','ibd']
                    gene['oddsratioibdlof'] = lofdict[genesym,'oddsratio','ibd']
                if (genesym,'AJnvar','cd') in lofdict:
                    gene['AJnvarcdlof'] = lofdict[genesym,'AJnvar','cd']
                    gene['AJcntacdlof'] = lofdict[genesym,'AJcnta','cd']
                    gene['AJcntucdlof'] = lofdict[genesym,'AJcntu','cd']
                    gene['FINnvarcdlof'] = lofdict[genesym,'FINnvar','cd']
                    gene['FINcntacdlof'] = lofdict[genesym,'FINcnta','cd']
                    gene['FINcntucdlof'] = lofdict[genesym,'FINcntu','cd']
                    gene['NFEnvarcdlof'] = lofdict[genesym,'NFEnvar','cd']
                    gene['NFEcntacdlof'] = lofdict[genesym,'NFEcnta','cd']
                    gene['NFEcntucdlof'] = lofdict[genesym,'NFEcntu','cd']
                    gene['totpvaluecdlof'] = lofdict[genesym,'totpvalue','cd']
                    gene['oddsratiocdlof'] = lofdict[genesym,'oddsratio','cd']
                if (genesym,'AJnvar','uc') in lofdict:
                    gene['AJnvaruclof'] = lofdict[genesym,'AJnvar','uc']
                    gene['AJcntauclof'] = lofdict[genesym,'AJcnta','uc']
                    gene['AJcntuuclof'] = lofdict[genesym,'AJcntu','uc']
                    gene['FINnvaruclof'] = lofdict[genesym,'FINnvar','uc']
                    gene['FINcntauclof'] = lofdict[genesym,'FINcnta','uc']
                    gene['FINcntuuclof'] = lofdict[genesym,'FINcntu','uc']
                    gene['NFEnvaruclof'] = lofdict[genesym,'NFEnvar','uc']
                    gene['NFEcntauclof'] = lofdict[genesym,'NFEcnta','uc']
                    gene['NFEcntuuclof'] = lofdict[genesym,'NFEcntu','uc']
                    gene['totpvalueuclof'] = lofdict[genesym,'totpvalue','uc']
                    gene['oddsratiouclof'] = lofdict[genesym,'oddsratio','uc']
...
def run_mrp(lof=True, missense=True, genes=None, fdr=5, phenidarr = ['ICD1462','ICD1463']):
    ...
    if genes != None:
     ...
     #   phenidarr = ["ICD1262_FH","ICD1111","ICD1387","ICD1226","ICD1138","ICD1220","ICDR10","ICDK62","ICD1452","ICD1505","ICD1606","ICDK40","ICDK29","ICDI84","ICD1154","ICD1474","ICDK21","ICDK80","ICDN39","ICDK57","ICDK52","ICD1113","ICDK30","ICD1466","ICDR19","ICD1453","ICD1464","ICDK20","ICD1538","ICD1502","ICDK59","ICD1416","ICDK92","ICD1463","ICDK35","ICDK81","ICD1190","ICD1139","ICD1462","ICD1313","ICDN30","ICDK85","ICDK58","ICD1459","ICDJ32","ICD1165","ICD1371"]
        #phenidarr = ['ICD1462', 'ICD1463']
        #phenidarr = ['ICD1462','ICD1463','ICD1261','ICD1111','ICD1387']
        ...
        for tmpc in range(1,6):
            [BIC, AIC, genedat] = mrpmm(betas,se, C, annotvec, gene_return, rsids, variant_ids,tmpc,key,C, numpy.linalg.inv(C),icd, fdr=fdr, niter=501,burn=50,thinning=1,verbose=True, outpath = './MRP_out/')
            bicarr.append(AIC)
            print(tmpc,BIC,AIC)
        cminarr = bicarr[1:]
...
#        if lbf > 1 and (lbf - lbf2) > 1 and lbf2 > 1:
        if lbf > 1:
            [BIC, AIC, genedat] = mrpmm(betas,se, C, annotvec, gene_return, rsids, variant_ids,clustminval,key,C, numpy.linalg.inv(C), icd, fdr=fdr, niter=2001,burn=1000,thinning=1,verbose=True, outpath = './MRP_out/')
...
def awesome():
    ...
    if datatype == 'gene':
        ...
        return redirect('/icd10/{}'.format(identifier))
...
@app.route('/icd10/<icd_str>')
def icd_page(icd_str):
    if not check_credentials():
        return redirect(url_for('login'))
    db = get_db()
    try:
        icdlabel = icd_str.strip('ICD')
...
def runMRP_page():
    ...
    if request.method == 'POST':
        ...
        print(genes)
        # Run MRP function
        [key,lbf,clusterval, genedat] = run_mrp(lof=lof, missense=missense, genes=genes, fdr=fdr)
        # Send results of MRP function to mrp page
