...
def get_icd(db, icd_id):
    return list(db.icd.find({'icd': icd_id}, fields={'_id': False}))

def get_icd_info(db, icd_id):
    return list(db.icd_info.find({'icd': icd_id}, fields={'_id': False}))

def get_variant_icd(db, xpos):
    return list(db.icd.find({'xpos': xpos}, fields={'_id': False}))
...
def get_variant(db, xpos):
    variant = db.variants.find_one({'xpos': xpos}, fields={'_id': False})
...
def get_variant_affy(db, affyid):
    variant = db.icd.find_one({'affyid': affyid}, fields={'_id': False})
    if variant is None or 'rsid' not in variant:
        return variant
    if variant['rsid'] == '.' or variant['rsid'] is None:
        rsid = db.dbsnp.find_one({'xpos': xpos})
...
def get_coverage_for_bases(db, xstart, xstop=None):
    ...
    for item in ret:
        item['has_coverage'] = '-log10pvalue' in item
        del item['xpos']
    return ret

def get_icdstats_for_bases(db, xstart, xstop=None):
    """
    Get the icdstats for the list of bases given by xstart->xstop, inclusive
    Returns list of icdstats dicts
    xstop can be None if just one base, but you'll still get back a list
    """
    if xstop is None:
        xstop = xstart
    icdstats = {
        doc['xpos']: doc for doc in db.base_icdstats.find(
            {'xpos': {'$gte': xstart, '$lte': xstop}},
            fields={'_id': False}
        )
    }
...
def get_awesomebar_result(db, query):
    ...
    We need to figure out what they meant - could be gene, variant, region, icd10
    ...
    if query.startswith('ICD'):
        # ICD10
        icd = get_icd(db, query)
        if icd:
    ...
    if variant['allele_count'] == 1:
        ...
        for af in AF_BUCKETS:
            if variant['allele_num'] == 0:
                metric = 'singleton'
            elif float(variant['allele_count'])/variant['allele_num'] < af:
                metric = af
                break
...
def get_variants_in_gene(db, gene_id):
    ...
    """
    variants = []
    for variant in db.variants.find({'genes_': gene_id}, fields={'_id': False}):
#    for variant in db.variants.find({'genes': gene_id}, fields={'_id': False}):
        ...
        variant['vep_annotations'] = [x for x in variant['vep_annotations'] if x['SYMBOL'] == gene_id]
...
def get_transcripts_in_gene(db, gene_id):
    """
    """
    print('find transcripts\n')
    print(gene_id)
    print(db.transcripts.find({'gene_id': gene_id}, fields={'_id': False}))
    print(db.transcripts.find_one({}, {'gene_id': 1, 'transcript_id': 1}))
...
def get_all_variants(db):
    """
    """
    variants = []
    for variant in db.variants.find(fields={'_id': False}):
        add_consequence_to_variant(variant)
        remove_extraneous_information(variant)
        variants.append(variant)
    return variants

def get_variants_in_chromosome(db, chr):
    """
    """
    variants = []
    for variant in db.variants.find({'chrom': chr}, fields={'_id': False}):
        add_consequence_to_variant(variant)
        remove_extraneous_information(variant)
        variants.append(variant)
    return variants
...
def get_variants_by_id(db, variant_ids):
    ...
    """
    variants = []
    for variant in db.variants.find({'xpos': {"$in":variant_ids}}, fields={'_id': False}):
        add_consequence_to_variant(variant)
        remove_extraneous_information(variant)
        variants.append(variant)
    return variants
...
def get_polygenic(db, icdarr, cutoff=0.001):
    ldpruned = open('../gbe_data/filters/all.prune.in','r').readlines()
    ldarr = []
    for line in ldpruned:
        line = line.rstrip()
        line = line.split()
        ldarr.append(line[0])
    return list(db.icd.find({'icd': {"$in": icdarr},'affyid': {"$in": ldarr}, 'stats.pvalue': { "$lt": cutoff } }, fields={'_id': False}))

def get_polygenicnull(db, icdarr, cutoff=0.5):
    ldpruned = open('../gbe_data/filters/all.prune.in','r').readlines()
    ldarr = []
    for line in ldpruned:
        line = line.rstrip()
        line = line.split()
        ldarr.append(line[0])
    return list(db.icd.find({'icd': {"$in": icdarr},'affyid': {"$in": ldarr}, 'stats.pvalue': { "$gt": cutoff }, 'stats.u95or': { "$lt": 1.05 }, 'stats.l95or': {"$gt": 0.95} }, fields={'_id': False}))
